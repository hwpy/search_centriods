...
# Модифицирован в части расчета расстояний, вместо Евклидова расстояния - метрика из distances_finder 
...
from packages.distances_finder import DistancesFinder
...
def _kmeans_single_elkan(
    ...
    center_half_distances = dist_finder.get_shortest_dist_by_car(centers) / 2
    ...
    for i in range(max_iter):
        ...
        center_half_distances = dist_finder.get_shortest_dist_by_car(centers_new) / 2
        distance_next_center = np.partition(
            np.asarray(center_half_distances), kth=1, axis=0
        )[1]
...
class _BaseKMeans(
    ...
    def _transform(self, X):
        ...
        return dist_finder.get_shortest_dist_by_car(X, self.cluster_centers_)
